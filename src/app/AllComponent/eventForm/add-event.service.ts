...
export class AddEventService {
  ...
  Add(formval : any): Observable<any>{
    ...
    return this.http.post(routeurls.BASE_API_URL + routeurls.EVENT_DAY__ADD_URL,formval,options)
  }
  getAll(): Observable<any> {
    return this.http.get<any>(routeurls.BASE_API_URL + routeurls.EVENT_DAY__ADD_URL).pipe(
      retry(3),
      map(res => {
        console.log(res);
        if (!res) {
  ...
  getbytemplateid(id: any): Observable<any> {
  ...
  bulkinsert(formval : any): Observable<any>{
    let headers = new HttpHeaders({
      'Content-Type': 'application/json'});
      let options = { headers: headers };
    //let header = headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
    return this.http.post(routeurls.BASE_API_URL + routeurls.EVENT_DAY__ADD_URL +  "/bulkinsert",formval,options)
  }
  ...
  getbyid(id: any): Observable<any> {
    return this.http.get<any>(routeurls.BASE_API_URL + routeurls.EVENT_DAY__ADD_URL+ "/getbyid", { params: new HttpParams().set('id', id) })
    .pipe(
      retry(3),
      map(res => {
        console.log(res);
  ...
  update(id: any, obj: any): Observable<any> {
    obj.Id = id;
    console.log(obj);
    return this.http.put<any>(routeurls.BASE_API_URL + routeurls.EVENT_DAY__ADD_URL , obj)
    .pipe(
      retry(3),
      map(res => {
        console.log(res);
  ...
  delete(id: any): Observable<any> {
    return this.http.delete<any>(routeurls.BASE_API_URL + routeurls.EVENT_DAY__ADD_URL, { params: new HttpParams().set('id', id) });
  }
}
